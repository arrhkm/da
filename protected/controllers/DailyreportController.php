...
class DailyreportController extends Controller
{
	...
	public function actionDelete($id)
	{
		...
		//$connect= yii::app()->db;
		//$sql= "DELETE FROM detilreport WHERE dailyreport_id=$id";
		//if ($connect->createCommand($sql)->query()){
		...
		if(!isset($_GET['ajax']))
			$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('adminstaff'));
	}
	...
	public function actionAdminStaff()
	{
		...
		$crt= new CDbCriteria();
		$crt->select = '*';
		$crt->order='date_report DESC';
		$crt->condition = 'employee_id=:id';
		$crt->params = array(':id'=>yii::app()->user->id);
		*/
	...
	public function actionDetilreport()
	{
    	...
    	$gJob= GroupJobtitle::model()->findByAttributes(array('employee_id'=>$user_id));
    	if (empty($gJob->employee_id)){
    		$message= "<h3>Silahkan setting Departemen dan Job Title Anda terlebih dahulu di menu STAFF->SETTING !</h3>";
    		//$error= 1;
    		$this->render('detilreporterror',array(		    	
		    	'message'=>$message,
		    	'errorr'=>true,
		    ));
    	}
			...
			if(isset($_POST['Detilreport']))
			{
		        ...
		        if($NewDetil->validate())
		        {
		            ...
		            //return;
		            $NewDetil->listjob= strtoupper($NewDetil->listjob);
		            if ($NewDetil->save())
		            {
	...
	public function actionUpdateDetil($id, $dailyreport_id)
	{
		...
		if(isset($_POST['Detilreport']))
		{
			...
			$detil->listjob= strtoupper($detil->listjob);
			if($detil->save())
			{
				$this->redirect(array('viewstaff','id'=>$dailyreport_id));
			}
